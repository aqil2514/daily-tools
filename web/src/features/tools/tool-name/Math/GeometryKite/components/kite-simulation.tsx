...
import {
  generateKiteProblem,
  KiteProblem,
} from "../utils/generate-kite-problem";
import { CheckCircle2, XCircle } from "lucide-react";
import { useLocale } from "next-intl";
import { i18nKiteSimulation } from "../i18n/kite-simulation";

export function KiteSimulation() {
  const locale = useLocale();
  const t = i18nKiteSimulation[locale];

  const [minValue, setMinValue] = useState(2);
  const [maxValue, setMaxValue] = useState(20);

  const [problem, setProblem] = useState<KiteProblem>(
    generateKiteProblem(minValue, maxValue)
  );
  ...
  function nextProblem() {
    setProblem(generateKiteProblem(minValue, maxValue));
    setAreaAnswer("");
    setPerimeterAnswer("");
    setChecked(false);
  }
        ...
        <div className="rounded-md border bg-muted/30 p-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Input
              ...
              value={maxValue}
              onChange={(e) =>
                setMaxValue(Number(e.target.value) || minValue)
              }
            />
          </div>
        ...
        {checked && (
          <div className="rounded-md border bg-muted/20 p-3 text-sm space-y-2">
            ...
            <p className="text-muted-foreground">
              {t.explanation.known}
              <br />
              d₁={problem.d1}, d₂={problem.d2}, a=
              {problem.a}, b={problem.b}
            </p>
...
function Feedback({
  ...
  label: string;
  ok: boolean;
  t: typeof i18nKiteSimulation[keyof typeof i18nKiteSimulation];
}) {
  return (
