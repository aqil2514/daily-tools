...
import {
  generateParallelogramProblem,
  ParallelogramProblem,
} from "../utils/generate-parallelogram-problem";
import { CheckCircle2, XCircle } from "lucide-react";
import { useLocale } from "next-intl";
import { i18nParallelogramSimulation } from "../i18n/parallelogram-simulation";

export function ParallelogramSimulation() {
  const locale = useLocale();
  const t = i18nParallelogramSimulation[locale];
  ...
  const [problem, setProblem] =
    useState<ParallelogramProblem>(
      generateParallelogramProblem(minValue, maxValue)
    );
...
function Feedback({
  ...
  label: string;
  ok: boolean;
  t: typeof i18nParallelogramSimulation[keyof typeof i18nParallelogramSimulation];
}) {
  return (
