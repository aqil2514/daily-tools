...
import {
  ...
  CircleProblem,
} from "../utils/generate-circle-problem";
import { CheckCircle2, XCircle } from "lucide-react";
import { useLocale } from "next-intl";
import { i18nCircleSimulation } from "../i18n/circle-simulation";

export function CircleSimulation() {
  const locale = useLocale();
  const t = i18nCircleSimulation[locale];
  ...
  // Current problem
  const [problem, setProblem] = useState<CircleProblem>(
    generateCircleProblem(minValue, maxValue)
  );

  // User answers
  const [areaAnswer, setAreaAnswer] = useState("");
  const [circumferenceAnswer, setCircumferenceAnswer] =
    useState("");
  const [checked, setChecked] = useState(false);

  const isAreaCorrect =
    Number(areaAnswer) === problem.correctArea;
  const isCircumferenceCorrect =
    Number(circumferenceAnswer) ===
    problem.correctCircumference;
  ...
  function nextProblem() {
    setProblem(generateCircleProblem(minValue, maxValue));
    setAreaAnswer("");
    setCircumferenceAnswer("");
    setChecked(false);
  }
      ...
      <div className="space-y-5">
        ...
        <div className="rounded-md border bg-muted/40 p-3 text-sm">
          ...
          <ul className="list-disc pl-5">
            ...
            <li>{t.question.circumference}</li>
          </ul>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Input
            placeholder={t.input.circumferencePlaceholder}
            value={circumferenceAnswer}
            disabled={checked}
            onChange={(e) =>
              setCircumferenceAnswer(e.target.value)
            }
          />
        </div>
        ...
        {checked && (
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            />
            <Feedback
              label={t.question.circumference}
              ok={isCircumferenceCorrect}
              t={t}
            />
          </div>
        )}
            ...
            <ul className="list-disc pl-5 text-muted-foreground space-y-1">
              <li>
                ...
                <span className="font-medium text-foreground">
                  L = π × r² = π × {problem.radius}² ={" "}
                  {problem.correctArea} cm²
                </span>
              </li>

              <li>
                {t.explanation.circumferenceIntro}
                <br />
                <span className="font-medium text-foreground">
                  K = 2 × π × r = 2 × π ×{" "}
                  {problem.radius} ={" "}
                  {problem.correctCircumference} cm
                </span>
              </li>
            </ul>
          </div>
...
/* Helper kecil agar rapi & konsisten */
function Feedback({
  label,
  ok,
  t,
  ...
  label: string;
  ok: boolean;
  t: typeof i18nCircleSimulation[keyof typeof i18nCircleSimulation];
}) {
  return (
