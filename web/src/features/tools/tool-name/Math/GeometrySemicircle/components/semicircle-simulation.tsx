...
import {
  generateSemicircleProblem,
  SemicircleProblem,
} from "../utils/generate-semicircle-problem";
import { CheckCircle2, XCircle } from "lucide-react";
import { useLocale } from "next-intl";
import { i18nSemicircleSimulation } from "../i18n/semicircle-simulation";

export function SemicircleSimulation() {
  const locale = useLocale();
  const t = i18nSemicircleSimulation[locale];

  const [minValue, setMinValue] = useState(2);
  const [maxValue, setMaxValue] = useState(20);

  const [problem, setProblem] =
    useState<SemicircleProblem>(
      generateSemicircleProblem(minValue, maxValue)
    );
      ...
      <div className="space-y-5">
        ...
        <div className="rounded-md border bg-muted/40 p-3 text-sm">
          ...
          <ul className="mt-2 list-disc pl-5">
            <li>
              {t.question.radius} ={" "}
              <strong>{problem.radius} cm</strong>
            </li>
          </ul>
            ...
            <ul className="list-disc pl-5 text-muted-foreground space-y-1">
              <li>
                ...
                <span className="font-medium text-foreground">
                  L = ½ × π × r² = ½ × π ×{" "}
                  {problem.radius}² ={" "}
                  {problem.correctArea.toFixed(2)} cm²
                </span>
              </li>
              ...
              <li>
                ...
                <span className="font-medium text-foreground">
                  ...
                  {problem.radius} + 2 ×{" "}
                  {problem.radius} ={" "}
                  {problem.correctPerimeter.toFixed(2)} cm
                </span>
              </li>
            </ul>
          </div>
...
function Feedback({
  ...
  label: string;
  ok: boolean;
  t: typeof i18nSemicircleSimulation[keyof typeof i18nSemicircleSimulation];
}) {
  return (
